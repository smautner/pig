...
def conservation(cov, ali):
    ...
    #print (cov)
    #print([ 1 if b == '2' else a  for a,b in zip(r,cov) ])
    return  np.mean(r), np.mean([ 1 if b == '2' else a  for a,b in zip(r,cov)  ])
...
def cov_sloppycov_disturbance_instem(stru,cov, ali, stacks,con):
    # % covariance in stems in stems   # with or without shorts removed
    # sloppy
    # # optionally remove fail line 

    # get indices of stems (all and long)
    hbonds = [ range(a,b+1) for a,b in stacks if b-a+1>2]
    hbonds = [a for b in hbonds for a in b]
    hbonds2 = [ range(a,b+1) for a,b in stacks]
    hbonds2 = [a for b in hbonds2 for a in b]

    pcov_large = sum([1 for a in hbonds if cov[a]=='2'])/len(hbonds)
    pvoc_all = sum([1 for a in hbonds2 if cov[a]=='2'])/len(hbonds2)
    
    # now we get the sloppy covariance 
    sl_large = get_sloppy(hbonds, ali, con)
    sl_all = get_sloppy(hbonds2, ali, con)

    # now we filter the most horrible line and do the stuff up there again. 
    weird = weirdo_detection(ali)
    ali2 = ali[[ a for a  in range(ali.shape[0]) if a not in weird]]
    
    return pcov_large,pvoc_all, sl_large, sl_all, checov(ali2,hbonds,con,cov), checov(ali2,hbonds2,con,cov)

def stemlength(stacks):
    s = [ b-a+1 for a,b in stacks  ]
    s.sort()
    return s[0],s[-2],s[-1]


def stemconservation(stacks,ali):
    stacks2 = [ (a,b) for a,b in stacks if b-a+1 > 2  ]
    return cons_stem(stacks,ali), cons_stem(stacks2,ali)
